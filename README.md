...
*Nota di manutenzione (14/08/2026): aggiornate le dipendenze di build (`marked`, `mustache`) per correggere vulnerabilità di sicurezza note (ReDoS in `marked`, XSS in `mustache`); adeguato `render.js` alla nuova API di `marked`. Sostituito anche il tool `gh-pages-deploy` (abbandonato e già non funzionante) con il pacchetto mantenuto `gh-pages` per `npm run deploy`. Deploy ora automatico: una GitHub Action ricostruisce e ripubblica il sito ad ogni push su `main`. Nessuna modifica al contenuto del tutorial.*
